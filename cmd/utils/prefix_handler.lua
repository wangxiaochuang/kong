...
local function gen_default_ssl_cert(kong_config, target)
  local ok, err = pl_dir.makepath(pl_path.join(kong_config.prefix
, "ssl"))
  if not ok then
    return nil, err
  end
...
local function gen_trusted_certs_combined_file(combined_filepath, paths)
  error("gen_trusted_certs_combined_file")
end
...
local function compile_conf(kong_config, conf_template)
  ...
  compile_env = pl_tablex.merge(compile_env, kong_config, true)
...
local function prepare_prefix(kong_config, nginx_custom_template_path, skip_write)
  ...
  do
    for _, target in ipairs({ "proxy", "admin", "status" }) do
      ...
      if ssl_enabled and #ssl_cert == 0 and #ssl_cert_key == 0 then
        ...
        -- kong-default.crt admin-kong-default.crt
  ...
  if kong_config.lua_ssl_trusted_certificate_combined then
    error("lua_ssl_trusted_certificate_combined")
  end
